...
def generate_test_I(sigma=0.1, L=3., nphotons=1000, noise_sigma=None, photon_energy=1.0): 
    ...
    for ii,d_sami in enumerate(d_sam):
        values[ii] = I_x_approx(d_sami, sigma)*photon_energy
        if noise_sigma is not None:
            ...
            while abs(v) > L/2.:
                ...
                v = I_x_approx(d_sami, sigma)*photon_energy
                v = np.random.normal(v, noise_sigma)
            values[ii] = v
    return values

I_arr = lambda nbins, photon_energy: np.linspace(-0.5/photon_energy, 1.5/photon_energy, nbins)
dI_arr = lambda nbins, photon_energy: (2.0/photon_energy)/(nbins-1)

def histogram(values, nbins, photon_energy=1.0): 
    dI = dI_arr(nbins, photon_energy)
    H_test, edges = np.histogram(values, nbins, range=((-0.5/photon_energy)-dI/2.,(1.5/photon_energy)+dI/2.))
    I_test = I_arr(nbins, photon_energy)
    return I_test, H_test

def generate_test_hist_I(sigma=0.1, L=3., nphotons=1000, noise_sigma=None, nbins=101, photon_energy=1.0):
    v = generate_test_I(sigma=sigma, L=L, nphotons=nphotons, noise_sigma=noise_sigma, photon_energy=photon_energy)
    I_test, H_test = histogram(v, nbins, photon_energy)
    return I_test, H_test


#_hist_I = lambda sigma, L, nphotons, nbins: f_I(I_arr(nbins), sigma, L) * dI_arr(nbins) * nphotons
_hist_I = lambda sigma, L, nphotons, nbins, photon_energy: f_I(I_arr(nbins, photon_energy), sigma, L) * (2./nbins) * nphotons
def hist_I(sigma, L, nphotons, noise_sigma=None, nbins=101, photon_energy=1.0):
    I = I_arr(nbins, photon_energy)
    dI = dI_arr(nbins, photon_energy)
    ...
    if noise_sigma is not None:
        ...
        tmp = np.linspace(-1., 1., nbins)
        G = gauss(tmp, noise_sigma)
        H = np.convolve(H, G/G.sum(), 'same')
    I = I_arr(nbins, 1.0)
    return I, H


def fit_hist(H, sigma=0.2, L=3, noise_sigma=0.1, photon_energy=1.0):
    nbins = len(H)
    nphotons = H.sum()
    x0 = [sigma, L, noise_sigma]
    func = lambda x: (hist_I(sigma=x[0], L=x[1], nphotons=nphotons, noise_sigma=x[2], nbins=nbins, photon_energy=photon_energy)[1] - H)/(np.finfo("float64").resolution + hist_I(sigma=x[0], L=x[1], nphotons=nphotons, noise_sigma=x[2], nbins=nbins, photon_energy=photon_energy)[1] + H)
